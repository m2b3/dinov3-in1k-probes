...
| `COMET_API_KEY` | `train` | Comet ML API key for live experiment tracking. If unset, training writes Comet offline logs under `$CHECKPOINTS_DIR/comet_offline/`. |
| `COMET_WORKSPACE` | `train` | Comet workspace name for live logging (pass via CLI `--comet-workspace` or env var) |
